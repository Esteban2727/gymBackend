...
export class GymUserController {
  ...
  @Get(':id')
  @ApiOperation({
    summary: 'Get users associated with a gym',
    description: 'Fetches users linked to a specific gym by ID.',
  })
  @ApiResponse({
    status: 200,
  ...
  async getUserOfGym(@Param('id') id: string) {
    console.log(id);
    const bringUserAssociateToGym =
      await this.gymUserServices.userAssociateToGym(id);
    return bringUserAssociateToGym;
  }

  @Post()
  @ApiOperation({
    summary: 'Associate a user to a gym',
    description: 'Links a user to a gym.',
  })
  @ApiResponse({
    status: 201,
  ...
  async AssocatetUserToGym(@Body() gymUserDto: GymUserDto) {
    ...
    const bringUserAssociateToGym =
      await this.gymUserServices.createAsocciateGym(idGym, idUser);
    return bringUserAssociateToGym;
  }
}
