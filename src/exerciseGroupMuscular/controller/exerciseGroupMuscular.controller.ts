...
export class ExerciseGroupMuscularController {
  constructor(
    private readonly exerciseGroupMuscularService: ExerciseGroupMuscularServices,
  ) {}
  @Get('getGroupMuscle')
  async getGrouoMuscle() {
    const ExerciseGroupMuscularServices =
      await this.exerciseGroupMuscularService.getGroupMuscularServices();
  ...
  async deleteByName(@Query('name') name: string) {
    const result =
      await this.exerciseGroupMuscularService.softDeleteBtName(name);
    return result;
  }
}
