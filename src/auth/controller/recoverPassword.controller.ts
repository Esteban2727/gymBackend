...
export class RecoverPasswordController {
  constructor(private readonly recoverPasswordServices: RecoverPasswordServices) {}

  @Post()
  @ApiOperation({ summary: 'Request password recovery', description: 'Sends a password recovery email to the user.' })
  @ApiResponse({ status: 200, description: 'Recovery email sent', schema: {
  ...
  @Get('reset-password')
  @ApiOperation({ summary: 'Reset user password', description: 'Resets user password using a valid token.' })
  ...
  async resetPassword(@Query('token') token: string, @Body() password: LoginDto) {
    if (!token) {
      throw new BadRequestException("Token es requerido");
    }
    const datas = await this.recoverPasswordServices.resetPassword(token, password.password);
    return datas;
  }
}
